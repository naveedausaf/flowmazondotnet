...
output "owner_connectionstring_for_psql" {
  ...
  # of the project.
  value     = "postgresql://${var.neon_owner_role}:${neon_role.owner_role.password}@${neon_endpoint.new_branch.host}/${var.neon_database_name}?sslmode=require&channel_binding=require"
  sensitive = true
}
